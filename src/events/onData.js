...
export const onData = (socket) => (data) => {
    ...
    const totlaHeaderLength = TOTAL_LENGTH + PACKET_TYPE_LENGTH; // = 5

    while(socket.buffer.length > totlaHeaderLength){
        // 전체의 길이
        ...
        if(socket.buffer.length >= length){
            ...
            const packet = socket.buffer.subarray(totlaHeaderLength, length);
            //자른 나머지 부분을 socket.buffer에 넣어줌.
            socket.buffer = socket.buffer.length.subarray(length);
            try{
                switch(packetType){
